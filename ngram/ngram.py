	...
	def compute_prediction(self, sentences, ngramType=1):
		...
		if(len(sentences) > 5):
			for index in range(0,len(sentences)):
				tempProbs = []
				for i, s in enumerate(sentences[index]):
					...
					if ngramType == 1:				
						if previousWords in self.ngrams:
							...
							tempProbs.append(float(fourgram)/float(histgram))
						else:
							tempProbs.append(0)	
					elif ngramType == 2:
						...
						tempProbs.append(bigram + 2.0 * trigram + 3.0 * fourgram)
					elif ngramType == 3:
						if previousWords in self.ngrams:
							histgram = self.ngrams[previousWords]
						else:
							histgram = 0					
						tempProbs.append(float(fourgram + 1)/float(histgram + self.V))
					elif ngramType == 4:
						if s in self.ngrams:
							c = self.ngrams[s]
						...
						tempProbs.append(float(occurs_once) / float(self.N))
					else:
						print "Wrong type"
				probs.append(sum(tempProbs))
		else:
