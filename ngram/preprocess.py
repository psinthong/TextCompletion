...
def preprocess(text, isTexts):
    if isTexts:
        ...
        # textPreprocessed = line_identifiers(remove_spaces(remove_names(remove_numbers(remove_determiner(remove_punctuation(remove_uppercase(sentence_per_line(text))))))))
    else:
...
if __name__ == "__main__":
    ...
    with open("preprocessed/train_set4.txt", 'w+') as out:
		out.write(train_set)

    # with open("lm_questions.txt", 'r') as questions, open("preprocessed/questions.txt", 'w+') as out:
    #     out.write('\n'.join(preprocess(questions.read().replace("<s> ", "").replace(" </s>", "").split('\n'), False)))
    # with open("lm_answers.txt", 'r') as answers, open("preprocessed/answers.txt", 'w+') as out:
